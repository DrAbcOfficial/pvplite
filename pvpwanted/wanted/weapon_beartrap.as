...
class weapon_beartrap : ScriptBasePlayerWeaponEntity, CBaseCustomWeapon
{
	void Spawn()
	{
		Precache();
...
{
	CBaseEntity@ pFind = g_EntityFuncs.FindEntityByClassname( null, "monster_beartrap" );
	while( pFind !is null)
	{
		monster_beartrap@ pBeartrap = cast<monster_beartrap@>(CastToScriptClass(pFind));
		if( pBeartrap !is null)
		{
			if ( pBeartrap.pev.owner is pOwner.edict() )
			{
				pBeartrap.pev.solid = SOLID_NOT;
				g_EntityFuncs.Remove( pBeartrap.self );
			}
		}
