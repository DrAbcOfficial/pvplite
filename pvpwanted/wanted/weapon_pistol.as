...
class weapon_pistol : ScriptBasePlayerWeaponEntity, CBaseCustomWeapon
{
	...
	void PistolFire( float flSpread, float flCycleTime, bool fUseAutoAim )
	{
		...
		m_pPlayer.FireBullets( 1, vecSrc, vecAiming, Vector( flSpread, flSpread, flSpread ), 8192, BULLET_PLAYER_CUSTOMDAMAGE, 0, m_iBulletDamage, pevAttacker:self.pev);
	...
	void Reload()
	{
		...
		self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + 2.8f;
	...
	void WeaponIdle()
	{
...

		{
			...
			if( bQuickFire )
			{
				...
				self.m_flTimeWeaponIdle = g_Engine.time + Math.RandomFloat( 2.0, 3.0 );
