...
class CTeslagun : ScriptBasePlayerWeaponEntity, CBaseCustomWeapon
{
		...
		if ( iDamage > 0 )
		{
			if ( tr.pHit !is null )
			{
				...
				if ( pHit !is null )
				{
					...
					pHit.TraceAttack( self.pev, iDamage, vecAiming, tr, DMG_GIB_CORPSE );
	...
	void UpdateLightning2( const Vector& in vecSrc, const Vector& in vecAiming, int iDamage )
	{
		...
		if ( iDamage > 0 )
		{
			if ( tr.pHit !is null )
			{
				...
				{
					g_WeaponFuncs.ClearMultiDamage();
					pHit.TraceAttack( self.pev, iDamage, vecAiming, tr, DMG_ENERGYBEAM );
					g_WeaponFuncs.ApplyMultiDamage( self.pev, self.pev );
				}
	...
	void UpdateLightning( const Vector& in vecSrc, const Vector& in vecAiming, int iDamage )
	{
		...
		if ( iDamage > 0 )
		{
			if ( tr.pHit !is null )
			{
				...
				{
					g_WeaponFuncs.ClearMultiDamage();
					pHit.TraceAttack( self.pev, iDamage, vecAiming, tr, DMG_SHOCK );
					g_WeaponFuncs.ApplyMultiDamage( self.pev, self.pev );
				}
				if ( pHit is null || pHit.IsBSPModel() )
				{
