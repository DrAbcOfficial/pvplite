...
class weapon_hlegon : ScriptBasePlayerWeaponEntity, CBasePVPWeapon
{
        ...
        switch( m_fireMode )
        {
            case FIRE_NARROW:
            {
                if ( self.pev.dmgtime < g_Engine.time )
                {
                    ...
                    {
                        pEntity.TraceAttack( self.pev, 5, vecDir, tr, DMG_ENERGYBEAM );
                    }
            ...
            case FIRE_WIDE:
            {
                if ( self.pev.dmgtime < g_Engine.time )
                {
                    ...
                    if ( pEntity.pev.takedamage > 0 )
                    {
                        pEntity.TraceAttack( self.pev, 10, vecDir, tr, ( DMG_ENERGYBEAM | DMG_ALWAYSGIB ) );
                    }
                    g_WeaponFuncs.ApplyMultiDamage( self.pev, self.pev );
                    
                    // radius damage a little more potent in multiplayer.
                    g_WeaponFuncs.RadiusDamage( tr.vecEndPos, self.pev, self.pev, 10/4, 128, CLASS_NONE, ( DMG_ENERGYBEAM | DMG_BLAST | DMG_ALWAYSGIB ) );
